...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "¡Hola! Soy Mariano. ¿Listo para mejorar tu juego? Consultame lo que quieras sobre mis clases o técnica de pádel." }
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-24 z-[60] flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-[350px] sm:w-[400px] h-[500px] bg-slate-900 rounded-3xl shadow-2xl border border-white/10 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5">
          <div className="bg-black p-4 flex justify-between items-center text-white border-b border-white/5">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-lime-500 rounded-full flex items-center justify-center font-bold text-black shadow-[0_0_10px_rgba(163,230,53,0.5)]">W</div>
              <div>
                <p className="font-bold text-sm">Chat con Mariano</p>
                <p className="text-[10px] text-lime-400">Respuesta inmediata</p>
              </div>
            </div>
      ...
      <button 
        ...
        className="w-16 h-16 bg-black border border-white/10 rounded-full shadow-[0_0_25px_rgba(163,230,53,0.2)] flex items-center justify-center text-lime-500 hover:scale-110 active:scale-95 transition-all duration-300 group"
      >
